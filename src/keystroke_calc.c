...
static void	st_key_left(t_image_mlx *img)
{
	...
	img->player.delta_y = sin(img->player.angle) * (img->blk_size / 30);
	img->player.delta_x = cos(img->player.angle) * (img->blk_size / 30);
}
...
void	hook(void *param)
{
	...
	if (mlx_is_key_down(img->mlx, MLX_KEY_RIGHT))
		st_key_right(img);
	if (mlx_is_key_down(img->mlx, MLX_KEY_SPACE))
		key_space(img);
	draw_cube(img);
}
